...
# Variables to manage TTS timing
last_said_time = time.time()
...
while True:
    ...
    # Flag to track if a name has been spoken recently
    name_spoken = False
    ...
    for (x, y, w, h) in faces:
        ...
        # Use TTS to say the name if not recently spoken
        if name != "Unknown" and not name_spoken:
            current_time = time.time()
            if current_time - last_said_time >= speak_interval:
                engine.say(f"Hello {name}")
                engine.runAndWait()
                last_said_time = current_time
                name_spoken = True
